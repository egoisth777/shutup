...
#
# One-line install:
#   curl -fsSL https://raw.githubusercontent.com/egoisth777/shutup/main/install.sh | bash
#   curl -fsSL https://raw.githubusercontent.com/egoisth777/shutup/main/install.sh | bash -s -- --all
#
# Local clone:
...
if ! command -v node >/dev/null 2>&1; then
  echo "shutup: Node.js (≥18) required. Install:" >&2
...
if [ "$NODE_MAJOR" -lt 18 ]; then
  echo "shutup: Node $NODE_MAJOR too old. Need Node ≥18." >&2
...
if ! command -v npx >/dev/null 2>&1; then
  echo "shutup: npx required (ships with Node ≥18). Reinstall Node.js." >&2
  exit 1
fi
